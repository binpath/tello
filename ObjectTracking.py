...
def stackImages(scale, imgArray):
  ...
  if rowsAvailable:
    for x in range_(_0, rows):
      for y in range(0, cols):
        if imgArray[x][y].shape[:2] == imgArray[0][0].shape[:2]:
          imgArray[x][y] = cv2.resize(imgArray[x][y], (0, 0), None, scale, scale)
        else:
          imgArray[x][y] = cv2.resize(imgArray[x][y], (imgArray[0][0].shape[1], imgArray[0][0].shape[0]), None, scale, scale)
        if len(imgArray[x][y].shape) == 2: imgArray[x][y]= cv2.cvtColor(_imgArray[x][y], cv2.COLOR_GRAY2BGR)
...
def getContours(img, imgContour):
  ...
  for cnt in contours:
    ...
    if area > areaMin:
      ...
      cv2.rectangle(imgContour, (x_,y_), (x + w_, y + h_), (0, 255, 0), 5)
      ...
      cv2.line(imgCointour, (int(frameWidth/2),int(frameHeight/2)), (cx,cy),(0, 0, 255), 3)
...
while True:
  ...
  result = cv2.bitwise_and(img,img, mask_=_mask)
  ...
  imgCanny = cv2.Canny(imgGray, threshold1, threshold1)
  kernel = np.ones((5,5))
  imgDil = cv2.dilate(imgCanny, kernel, iterations=1)
  getContours(imgDil, imgContour)
  display(imgContour)

  stack = stackImages(0.7,([img,result],[imgContour]))
